...
class Evaluator:
    ...
    def eval_all(self, data, save_path, save_npz_path, img_name):
        opt = self.opt
        self.netG.eval()
        gen_mesh_udf_all(opt, self.netG, self.cuda, data, save_path, save_npz_path, img_name, num_steps=opt.num_steps)
...
if __name__ == '__main__':
    ...
    for subject in var_subjects:
        ...
        for vid in yaw_list:
            image_path = os.path.join(opt.dataroot, 'RENDER', subject, '%d_%d_%02d.jpg' % (vid, 0, 0))
            mask_path = os.path.join(opt.dataroot, 'MASK', subject, '%d_%d_%02d.png' % (vid, 0, 0))
            data = evaluator.load_image(image_path, mask_path)
            evaluator.eval_all(data, save_path, save_npz_path, '%d_%d_%02d' % (vid, 0, 0))
