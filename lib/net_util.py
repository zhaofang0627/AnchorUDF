...
def conv2d(in_planes, out_planes, kernel_size=3, stride=1, norm='batch'):
    if norm == 'batch':
...
def fc(nc_inp, nc_out, batch_norm=True):
    if batch_norm:
        return nn.Sequential(
            nn.BatchNorm1d(nc_inp),
            nn.ReLU(inplace=True),
...
def fc_stack(nc_inp, nc_out, nlayers, use_bn=True):
    ...
    for l in range(nlayers):
        modules.append(fc(nc_inp, nc_out, use_bn))
        nc_inp = nc_out
