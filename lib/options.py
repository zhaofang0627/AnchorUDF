...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        g_test.add_argument('--test_mask_folder_path', type=str, default=None, help='the folder of test mask')

        # Sampling related
        g_sample = parser.add_argument_group('Sampling')
        g_sample.add_argument('--sigma', nargs='+', default=[0.08, 0.02, 0.003], type=float, help='perturbation standard deviation for positions')
        g_sample.add_argument('--sample_distribution', nargs='+', default=[0.01, 0.49, 0.5],  type=float)
        ...
        # hg filter specify
        g_model.add_argument('--num_stack', type=int, default=5, help='# of hourglass')
        g_model.add_argument('--num_hourglass', type=int, default=4, help='# of stacked layer of hourglass')
        ...
        g_model.add_argument('--num_stack_hd', type=int, default=1, help='# of hourglass')
        g_model.add_argument('--num_hourglass_hd', type=int, default=6, help='# of stacked layer of hourglass')
        g_model.add_argument('--hourglass_dim_hd', type=int, default='16', help='16 | 256 | 512')

        # Classification General
        g_model.add_argument('--mlp_dim', nargs='+', default=[291, 1024, 512, 256, 128, 1], type=int,
                             help='# of dimensions of mlp')

        g_model.add_argument('--mlp_dim_hd', nargs='+', default=[272, 512, 256, 128, 1], type=int,
                             help='# of dimensions of mlp')
        g_model.add_argument('--merge_layer', type=int, default=0)
        ...
        parser.add_argument('--depth_z', action='store_true', help='if using z feat')
        parser.add_argument('--coord_xyz', action='store_true', help='if using xyz coordinate')
        parser.add_argument('--grad_constraint', action='store_true', help='if constraining spatial gradient norm as 1')
        parser.add_argument('--backbone_detach', action='store_true', help='if detaching backbone')
        parser.add_argument('--max_dist', type=float, default=0.2, help='maximal regressed distance')

        parser.add_argument('--coord_xyz_hd', action='store_true', help='if using xyz feat for hd')
        parser.add_argument('--joint_train', action='store_true', help='if joint training')
        ...
        parser.add_argument('--filter_val', type=float, default=0.006, help='threshold of filter value')
        parser.add_argument('--num_steps', type=int, default=10, help='how many steps to iterate during testing')
        parser.add_argument('--num_gen_points', type=int, default=900000, help='how many points to generate during testing')
        parser.add_argument('--mgpu', action='store_true', help='if using multiple GPUs')
        ...
        parser.add_argument('--load_netMR_checkpoint_path', type=str, default=None, help='path to save checkpoints')
        ...
        parser.add_argument('--demo_mask_name', type=str, help='path for input mask')
        parser.add_argument('--demo_img_name', type=str, help='path for input image')
